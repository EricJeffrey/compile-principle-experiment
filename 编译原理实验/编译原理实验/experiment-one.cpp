...
void Exp1WordTuple::printWord(ostringstream &scout) {
    if (typeCode == IDv$) {
        for (int i = 2; i <= 26; i++) {
    ...
    for (int i = 0; i < 26 - len; i++) 
        scout << ' ';
    scout << '\t' << "typecode: " << typeCode << '\t';
    if (typeCode >= 27)
        scout << oneCodeToStr[typeCode] << endl;
    else
        scout << keyWordDesc << endl;
}
Exp1WordTuple::~Exp1WordTuple() {
    
}

Exp1WordAnalyzer::Exp1WordAnalyzer(string &data) {
    string sourceCodeInput = data;
    ...
    for (int i = 0; i < wordsLen; i++) {
        wordParseOutput[i].printWord(scout);
    }
}
    ...
    while (i < len) {
        ...
        if (isDigit(ch)) {
            ...
            if (tmpch == '.') {
                ...
                if (tmpch == 'e' || tmpch == 'E') {
                    ...
                    if (isDigit(tmpch)) {
                        ...
                        errorExit(u8"科学计数法至少应当有一个指数位", scout);
                    }
                }
                //确定是实数
                else {
            ...
            //是整数
                ...
                errorExit(u8"整数或实数后面不能跟非空格", scout);
            }
        }
        //字符
        if (ch == '\'') {
            if (i + 1 < len) {
                ...
                if (tmpch == '\\' && +3 < len && input[i + 3] == '\'') {
                    ...
                    errorExit(u8"不可显示字符或无字符结束符", scout);
                }
            }
            else {
                errorExit(u8"无字符结束符", scout);
            }
        }
        //字符串
        if (ch == '"') {
        ...
        if (ch == '#') {
            if (i + 7 < len && input.substr(i, 8) == oneCodeToStr[0]) {
                ...
                while (input[k] != '>' && input[k] != 0 && input[k] != EOF)
                    k++;
                if (k - i >= 23)
                    errorExit(u8"头文件包含过长或缺少包含结束符>", scout);
                if (input[k] == '>')
                    k++;
                i = k;
            }
                ...
                errorExit(u8"不是宏定义也不是头文件包含", scout);
            }
        }
        //+ ++ +=
        if (ch == '+') {
...
string Exp1WordAnalyzer::getOutputStr() {
    return scout.str();
}
